---
title: AI 面试题专项：技术专家与架构师
type: topic
status: active
tags:
  - ai/interview
  - ai/architecture
created: 2026-04-12
updated: 2026-04-12
---

# AI 面试题专项：技术专家与架构师

> 这页现在作为 **总入口**。完整题库已经升级为子模块：[[../10-AI-Interview-Kit/专题总览|AI Interview Kit 专题总览]]。

## 为什么要从单页升级到子模块

- 单页适合速览，不适合持续扩展与复盘。
- 面试准备需要“题库 + 决策题 + 训练节奏 + 恢复入口”。
- 真正的高级岗位准备还需要“开场、自我定位、项目深挖、团队与业务沟通”。
- 因此把内容拆成可维护结构，避免题目越积越乱。

## 子模块入口

- 总览：[[../10-AI-Interview-Kit/专题总览|AI Interview Kit 专题总览]]
- 问题导航：[[../10-AI-Interview-Kit/面试问题导航|面试问题导航]]
- 决策导航：[[../10-AI-Interview-Kit/面试决策导航|面试决策导航]]
- 题库索引：[[../10-AI-Interview-Kit/05-Question-Banks/题库索引|题库索引]]
- 资深表达总纲：[[../10-AI-Interview-Kit/06-Answer-Playbooks/资深候选人表达总纲|资深候选人表达总纲]]
- 完整蓝图：[[../10-AI-Interview-Kit/完整面试题蓝图：如果是我会怎么准备|完整面试题蓝图：如果是我会怎么准备]]
- 详细答案：[[../10-AI-Interview-Kit/06-Answer-Playbooks/高频题详细答案（专家版）|高频题详细答案（专家版）]]
- 角色话术：[[../10-AI-Interview-Kit/06-Answer-Playbooks/角色版回答策略：技术专家、架构师、负责人|角色版回答策略]]
- 追问演练：[[../10-AI-Interview-Kit/07-Drills/追问剧本与反问模板|追问剧本与反问模板]]
- 错误纠偏：[[../10-AI-Interview-Kit/07-Drills/常见错误回答与修正|常见错误回答与修正]]
- 进度：[[../10-AI-Interview-Kit/学习进度|学习进度]]
- 恢复：[[../10-AI-Interview-Kit/恢复笔记|恢复笔记]]

## 现在的完整题库分区

1. [[../10-AI-Interview-Kit/05-Question-Banks/00-开场与项目深挖题库|开场与项目深挖题库]]
2. [[../10-AI-Interview-Kit/05-Question-Banks/01-LLM基础与训练题库|LLM基础与训练题库]]
3. [[../10-AI-Interview-Kit/05-Question-Banks/02-推理与系统架构题库|推理与系统架构题库]]
4. [[../10-AI-Interview-Kit/05-Question-Banks/03-Agent与工具调用题库|Agent与工具调用题库]]
5. [[../10-AI-Interview-Kit/05-Question-Banks/04-RAG与数据工程题库|RAG与数据工程题库]]
6. [[../10-AI-Interview-Kit/05-Question-Banks/05-评测与上线治理题库|评测与上线治理题库]]
7. [[../10-AI-Interview-Kit/05-Question-Banks/06-安全与合规题库|安全与合规题库]]
8. [[../10-AI-Interview-Kit/05-Question-Banks/07-团队与业务沟通题库|团队与业务沟通题库]]
9. [[../10-AI-Interview-Kit/05-Question-Banks/08-系统设计与场景题库|系统设计与场景题库]]
10. [[../10-AI-Interview-Kit/05-Question-Banks/09-成本、性能与容量规划题库|成本、性能与容量规划题库]]
11. [[../10-AI-Interview-Kit/05-Question-Banks/10-项目复盘与事故题库|项目复盘与事故题库]]

## 示例高频题（用于热身）

- 如何保证 Function Calling 可靠性？
- 多轮对话 KV Cache 爆了怎么办？
- RAG 的 Query 改写怎么做？
- SFT 和 RLHF 的本质区别？
- 多 Agent 如何动态派生与协同？

> 现在整套内容不再只是知识题，而是覆盖 `开场 -> 项目深挖 -> 技术题 -> 系统设计 -> 成本与容量 -> 架构治理 -> 事故复盘 -> 团队与业务沟通 -> 追问演练` 的完整面试流程。

## 关联主题

- [[Agent]]
- [[Tool Use]]
- [[RAG]]
- [[Long Context]]
- [[Multi-Agent Systems]]
- [[Evaluation]]
- [[MLOps 与 LLMOps]]
- [[AI Security]]
