...
- 如果你在看 Agent Runtime 深水区，优先按：`Agent Runtime Architecture -> Session and Memory Design -> Background Agents -> Task Success and Failure Recovery -> Agent Runtime 失败模式、事故复盘与 Postmortems -> Runtime 发布门槛、灰度与 Blast Radius 控制 -> Plugin、Memory 与 Background Task 失效模式 -> Agent Runtime 失败、发布与恢复图` 这条图线读
